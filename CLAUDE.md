...
- **Multi-GPU DDP strategy — must pair `find_unused_parameters=true` with `static_graph=true`.** The four confidence configs currently ship `strategy: ddp_find_unused_parameters_false`, which works on a single GPU but crashes step-1 backward on multi-GPU DDP. Two failures compose: (i) the frozen trunk has parameters that don't feed the head's loss (`PaeHead` consumes `z` only, `PlddtHead` consumes `s` only — neither uses every trunk param), so `find_unused_parameters=false` raises "parameters that were not used in producing the loss"; (ii) the trunk's pair-update layers use reentrant `torch.utils.checkpoint`, so with `find_unused_parameters=true` alone DDP's reducer marks the same checkpointed param ready twice and raises "Parameter at index N with name `head.trunk.pair_update_layers.*.linear_2.weight` has been marked as ready twice". Both flags together fix it. Override pattern via Hydra CLI: `~trainer.strategy +trainer.strategy._target_=lightning.pytorch.strategies.DDPStrategy +trainer.strategy.find_unused_parameters=true +trainer.strategy.static_graph=true`. The cleaner long-term fix is to flip the default in [configs/confidence/](configs/confidence/) to a structured `DDPStrategy` block — pending follow-up PR. Generalisation of the multi-head wrapper memo: `static_graph=true` is required for **any** confidence-distill multi-GPU run, not only when `loss_weights` zeroes a head.
