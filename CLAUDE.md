...
- **Trunk hook.** `LocalLatentsTransformer` has an opt-in `expose_intermediates: bool = False` kwarg. When `True`, `forward(input)` adds `nn_out["trunk_intermediates"]` carrying `(s, z, local_latents, mask, orig_mask, n_orig)` — `local_latents` is the pre-trim 8-dim per-residue side-chain + AA latent (added in PR #21). Default `False` is bit-identical to the legacy forward (enforced by `tests/regression/test_flow_matching_loss_unchanged.py` against a baseline fixture).
...
- **Joint training via `MultiHeadConfidence` wrapper — shipped in PR #10.** Lives at [src/proteinfoundation/nn/confidence/multi_head.py](src/proteinfoundation/nn/confidence/multi_head.py). Registered as `multi_head`. Holds an `nn.ModuleDict` of child heads, runs the trunk once per batch, dispatches the same `(s, z, mask, cond, local_latents)` to every child's `_predict`, and returns `{head_name: prediction}`. `local_latents` is the additional 8-dim per-residue input added in PR #21; the shared `ConfidenceTrunk` projects + LN's it and adds as a mask-zeroed residual to `s` at trunk entry, so heads consume the latent-enriched `s` (and pair-update-coupled `z`) without changing their own `_predict` signature.
- **LayerNorm-bias-leak invariant — applies to any masked additive residual past `Linear → LayerNorm`.** Whenever an auxiliary signal is projected into a masked sequence representation via `Sequential(Linear(d_in, d_token, bias=False), LayerNorm(d_token))` and added as a residual, the post-LN output **must** be multiplied by `mask_f` before the add — even if the upstream input is zero at padded positions. At init `beta = 0` makes `LN(0) = 0` look fine, but once training drifts `beta` off zero, `LN(0) = beta`: the trained bias vector lands in padded `s` rows, propagates into pair features via `PairReprUpdate` outer products + triangle multiplication, and contaminates valid `(i, j)` cells through summation over padded `k`. Valid-cell attention logits become coupled to padding length, which is a silent training-step-dependent correctness bug. The reference implementation is the `local_latents` residual at [src/proteinfoundation/nn/confidence/base.py:146-147](src/proteinfoundation/nn/confidence/base.py#L146-L147); the regression contract is [tests/integration/confidence/test_confidence_trunk_local_latents_proj.py::test_mask_multiply_guards_against_trained_layernorm_bias_leak](tests/integration/confidence/test_confidence_trunk_local_latents_proj.py), which writes `LN.bias = 0.3` (simulating drift) and asserts two forwards differing only at padded positions are bit-identical at every position. Any future projection of auxiliary trunk intermediates (chain-id embedding, additional latents, positional encoding) into `s` or `z` must follow the same `* mask` after the LN, and pin the invariant with a non-init test. `MultiHeadLoss` at [src/proteinfoundation/nn/confidence/_losses.py](src/proteinfoundation/nn/confidence/_losses.py) aggregates `total = sum(w_i * L_i)` with per-task scalars logged separately. Two weighting layers, kept separate in Hydra: (i) **within-head** `loss.ce_weight` / `loss.ev_weight` (the existing pLDDT recipe family); (ii) **across-head** `loss_weights: {head: scalar}` (multiplies the head's total loss before summing with siblings; `weight: 0.0` is a hard short-circuit — skip the head's loss call entirely). A head whose entire batch mask is False contributes exactly zero, never NaN-from-empty-mean. The wrapper asserts `expected_trunk_eval_t` matches across children at construction AND that every child has a non-empty `output_name_root`; a head needing a different `t` cannot live under this wrapper. Single-head runs (`PlddtHead`-only on SwissProt, `PaeHead`-only on Teddymer for the clean reference number) keep working unchanged — joint training is one Hydra config flip away, not the default. **DDP footgun:** `weight: 0.0` disables the loss but the child's `_predict` still runs in the wrapper's `forward`, so the child's parameters enter the autograd graph with no gradient — `find_unused_parameters=False` raises at backward. Working escape hatches: (a) remove the child from `children:`, (b) `DDP(static_graph=True)`. The wrapper emits a `RuntimeWarning` at construction listing offending heads.
