...
- **Pair-repr symmetrisation — MUST REVISIT in the next PR.** `ConfidenceTrunk.forward` (`src/proteinfoundation/nn/confidence/base.py:129`) currently ends with `z = (z + z.transpose(-3, -2)) / 2.0` — an explicit `(i, j) ↔ (j, i)` symmetrisation projection on the pair representation. This was the right default for the pLDDT head shipped in PR-3..PR-6 because every planned head at that time was symmetric in `(i, j)` (PDE, ipLDDT, ipTM — all undirected pair quantities). The user has flagged that **future reward heads may be asymmetric** in the residue-pair relation (e.g. AF2-style directional PAE, where `PAE(i, j)` is the error of residue `j`'s position when aligned on residue `i`'s frame — `PAE(i, j) != PAE(j, i)`). When the first asymmetric head lands, this line **must move down** into per-head `_predict` so symmetric heads (PDE, ipLDDT, ipTM, current pLDDT) can opt in and asymmetric heads can opt out. Concrete options: (a) add a `symmetrise_z: bool = True` flag on `ConfidenceTrunk.__init__`; (b) remove the symmetrisation from the trunk entirely and require every concrete head to symmetrise inside `_predict`. Option (b) is cleaner long-term; option (a) is a smaller diff. Decide when the first asymmetric head is specified, before its training run.
